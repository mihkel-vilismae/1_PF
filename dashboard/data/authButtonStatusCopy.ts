...
export const AUTH_BUTTON_STATUS_COPY = Object.freeze({
  ...
  'new-auth-check-login': {
    ...
    realityState: 'real',
    backendState: 'real',
    mutates: 'Reads the new auth session status without starting login, provider proof, or the existing login card status route.',
    successCriteria: 'Green means the passive new auth status check completed; the returned state still decides whether the account is authenticated.',
    realityReason: 'Production-real status UI boundary for the new auth endpoint family.',
    backendReason: 'Targets real backend route GET /api/auth/new/status in passive mode. It observes active login/session evidence without old status endpoints and without starting provider proof.',
    statuses: {
      neutral: 'Ready to check the current new auth session state.',
      running: 'Checking new auth status through GET /api/auth/new/status in passive mode.',
      success: 'The passive new auth status check completed.',
  ...
  'new-auth-generate-artifact-pack': {
    ...
    endpoint: 'POST /api/auth/new/artifacts/generate',
    operationType: 'new auth sanitized artifact generation',
    realityState: 'real',
    backendState: 'real',
    mutates: 'Writes a sanitized evidence pack under debug_artifacts/auth while excluding raw provider output and session file contents.',
    successCriteria: 'Green only when the backend reports a generated artifact pack with redaction checks preserved.',
    realityReason: 'Production-real debugging boundary for NEW AUTH evidence collection; it does not prove login success by itself.',
    backendReason: 'Targets real backend route POST /api/auth/new/artifacts/generate and delegates redaction to the backend artifact pack service.',
    statuses: {
      neutral: 'Ready to generate a sanitized NEW AUTH evidence pack.',
      running: 'Generating a sanitized NEW AUTH evidence pack.',
      success: 'A sanitized NEW AUTH evidence pack was generated.',
      failed: 'Artifact pack generation failed or returned an unsafe/unclear result.',
      blocked: 'Artifact pack generation was blocked by backend guardrails.',
      pending: 'Artifact pack generation returned a non-final state.',
      providerDependent: GENERIC_STATUS_COPY.providerDependent,
    },
  },
  'new-auth-list-artifact-packs': {
    label: 'List auth evidence packs',
    endpoint: 'GET /api/auth/new/artifacts',
    operationType: 'new auth sanitized artifact listing',
    realityState: 'real',
    backendState: 'real',
    mutates: 'Reads artifact pack metadata only and does not expose raw provider output or session file contents.',
    successCriteria: 'Green when the backend returns the existing sanitized artifact pack list.',
    realityReason: 'Production-real debugging inventory boundary for generated NEW AUTH evidence packs.',
    backendReason: 'Targets real backend route GET /api/auth/new/artifacts and returns safe metadata only.',
    statuses: {
      neutral: 'Ready to list sanitized NEW AUTH evidence packs.',
      running: 'Listing sanitized NEW AUTH evidence packs.',
      success: 'Sanitized NEW AUTH evidence pack metadata was returned.',
      failed: 'Artifact pack listing failed or returned an unsafe/unclear result.',
      blocked: 'Artifact pack listing was blocked by backend guardrails.',
      pending: 'Artifact pack listing returned a non-final state.',
      providerDependent: GENERIC_STATUS_COPY.providerDependent,
    },
  },
  'verify-icloudpd': {
