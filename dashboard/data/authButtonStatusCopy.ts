...
export const AUTH_BUTTON_STATUS_COPY = Object.freeze({
  'new-auth-verify-icloudpd': {
    ...
    mutates: 'Calls only the new auth endpoint family; Slice 1 provides frontend wiring before provider execution is implemented.',
    successCriteria: 'Green only when the new endpoint later proves iCloudPD is present and executable.',
    realityReason: 'Production-real UI boundary for new auth; it is intentionally separate from the existing login card.',
    backendReason: 'Targets POST /api/auth/new/verify-icloudpd. Slice 1 does not reuse old endpoints and expects later slices to provide backend behavior.',
    statuses: {
  ...
  'new-auth-login-using-env': {
    ...
    realityState: 'real',
    backendState: 'missing',
    mutates: 'Starts the new auth login flow and opens the modal shell; secrets remain server-side.',
    successCriteria: 'Green only when the new endpoint later proves a real authenticated session.',
    realityReason: 'Production-real auth UI boundary. Slice 1 provides modal/2FA UI without faking login success.',
    backendReason: 'Targets POST /api/auth/new/login and POST /api/auth/new/submit-2fa. It must not use old auth routes.',
    statuses: {
  ...
  'new-auth-check-login': {
    ...
    backendReason: 'Targets GET /api/auth/new/status. Slice 1 deliberately avoids old status endpoints.',
    statuses: {
  ...
  'new-auth-logout-session': {
    ...
    successCriteria: 'Green only when the new endpoint later confirms local session data was removed or no longer active.',
    realityReason: 'Production-real cleanup UI boundary; it must not call old logout endpoints.',
    backendReason: 'Targets POST /api/auth/new/logout. Slice 1 provides frontend wiring before cleanup implementation.',
    statuses: {
  ...
  'new-auth-session-files': {
    ...
    backendReason: 'Targets GET /api/auth/new/session-files. It must return metadata only and never expose session contents.',
    statuses: {
