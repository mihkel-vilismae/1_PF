...
import {
  ...
  generateNewAuthArtifactPack,
  listNewAuthArtifactPacks,
  fetchNewAuthStatus,
...
export function createRuntimeTruthNewAuthActions({ patchState, pushHistory, pushLog, setStatus, openModal, guards }) {
  ...
  // Generates a sanitized NEW AUTH evidence pack for local debugging.
  async function generateArtifactPackAction() {
    return runNewAuthBackendAction({
      buttonKey: 'new-auth-generate-artifact-pack',
      operation: 'Generate auth evidence pack',
      endpoint: NEW_AUTH_ENDPOINTS.generateArtifactPack,
      execute: generateNewAuthArtifactPack,
      duplicateMessage: '1A-STASH-OFF artifact pack generation is already running; duplicate start was blocked.',
      resultTarget: 'artifactPackResult',
    });
  }

  // Lists sanitized NEW AUTH evidence packs already generated by the backend.
  async function listArtifactPacksAction() {
    return runNewAuthBackendAction({
      buttonKey: 'new-auth-list-artifact-packs',
      operation: 'List auth evidence packs',
      endpoint: NEW_AUTH_ENDPOINTS.listArtifactPacks,
      execute: listNewAuthArtifactPacks,
      duplicateMessage: '1A-STASH-OFF artifact pack listing is already running; duplicate start was blocked.',
      resultTarget: 'artifactPackListResult',
    });
  }
  ...
  return {
    ...
    logoutAction,
    sessionFilesAction,
    generateArtifactPackAction,
    listArtifactPacksAction,
  };
}
...
function classifyNewAuthButtonStatus(buttonKey, payload, transportOutcome) {
  ...
  if (buttonKey === 'new-auth-generate-artifact-pack') return payload?.ok === true && payload?.artifact ? 'success' : 'pending';
  if (buttonKey === 'new-auth-list-artifact-packs') return payload?.ok === true && Array.isArray(payload?.packs) ? 'success' : 'pending';
...
function buttonKeyForNewAuthEndpoint(endpointPath) {
  ...
  if (endpointPath === NEW_AUTH_ENDPOINTS.generateArtifactPack.path) return 'new-auth-generate-artifact-pack';
  if (endpointPath === NEW_AUTH_ENDPOINTS.listArtifactPacks.path) return 'new-auth-list-artifact-packs';
