...
|------|------------|---------|--------|----------------------|------------------|----------|----------|
| A Init | `dashboard/views/initView.js`, `dashboard/app.js`, `dashboard/services/runtimeTruth.js`, `dashboard/services/initService.js`, `dashboard/services/apiClient.js`, `server/index.js`, `server/scripts/sqlite_admin.py`, `docs/VIEW_A_INIT.md`, `docs/13_FRONTEND_BACKEND_CONTRACT.md` | Prepare configuration, database, and cron readiness before tests or real runs. | Partial | A now has real env verification, real SQLite status/inspect/delete/recreate-empty endpoints, and destructive-action confirmation. The remaining gap is cron: the backend endpoints exist, but they intentionally return blocked/error responses because the Windows environment and 5-second scheduler contract cannot honestly map to standard cron. | Resolve the scheduler contract for 3A, decide whether A should preload status on entry, and refine the env/config schema assumptions if product requirements differ from the checked-in `.env`. | High | Cron remains blocked by a real contract/platform contradiction, and the env verification rules are currently derived from the checked-in `.env` rather than a separately approved config spec. |
...
### 1. Purpose
This area is meant to validate installation readiness before any test or real runtime starts. It now includes a repo-local backend slice for env verification and SQLite file operations plus frontend confirmation for destructive DB actions. The remaining incomplete part is cron, where the endpoint surface exists but the backend correctly reports a blocked contract rather than faking support.
...
| `server/index.js` | full file | Implements `/api/init/*` endpoints for env verification, DB status/inspect/delete/recreate-empty, and blocked cron responses. | This is the repo-local A backend implementation. |
...
| Business logic | Partial | `server/index.js`; `server/scripts/sqlite_admin.py` | Env verification and DB file operations are real, but cron remains blocked by an unresolved scheduler contract. |
| Error handling | Implemented | `dashboard/services/apiClient.js`; `dashboard/services/runtimeTruth.js`; `server/index.js` | Transport failures, HTTP errors, missing DB files, missing confirmation payloads, and blocked cron semantics are all surfaced honestly. |
| Backend integration | Partial | `server/index.js`; `docs/13_FRONTEND_BACKEND_CONTRACT.md` | Repo-local backend integration now exists for most of A, but 3A is intentionally incomplete because the contract cannot honestly be satisfied yet. |

### 4. Placeholder Logic To Replace

| Placeholder or Mock Behavior | Evidence | Why It Is Placeholder | Required Real Behavior |
|------------------------------|----------|-----------------------|------------------------|
| 3A cron endpoints are real but currently blocked because the documented scheduler model conflicts with the platform and with standard cron semantics. | `server/index.js`; `docs/09_CRON_AND_WATCHDOG.md` lines 14-19 | Returning success here would be misleading because Windows lacks cron and the documented 5-second schedule is not representable in standard cron. | Decide whether 3A should target Unix crontab, Windows Task Scheduler, or a different scheduler abstraction. |
...
| Validation | Partially implemented | `server/index.js`; `dashboard/app.js` | Env values are validated structurally and destructive DB actions require explicit confirmation, but auth and cron policy validation remain unresolved. |
...
| 6 | Resolve 3A scheduling semantics instead of faking cron support. | `server/index.js`, `docs/09_CRON_AND_WATCHDOG.md`, `docs/13_FRONTEND_BACKEND_CONTRACT.md` | Product/platform decision | Open |
| 7 | Introduce an initial-read or refresh path if A should show readiness state before a button click. | `dashboard/app.js`, `dashboard/services/runtimeTruth.js`, possible backend additions | Product decision and endpoint support | Open |

### 7. Unknowns and Blockers

| Issue | Evidence | Why It Matters | What Is Needed To Resolve It |
|-------|----------|----------------|------------------------------|
| The cron contract is internally inconsistent for this environment. | `docs/09_CRON_AND_WATCHDOG.md` lines 14-19; `server/index.js` blocked cron responses | Standard cron is unavailable on Windows and cannot express the documented 5-second ticks anyway. | Decide on the real scheduler target for 3A and update the contract accordingly. |
...
|------|------|-----------|-------------------------|--------|
| 1 | A Init | High | Resolve the 3A scheduler target, then implement cron/status/install semantics that match the real platform and timing model. | Env and DB backend work are now implemented; the remaining A gap is the scheduler contract itself. |
...
|------|----------|--------|-----------|
| A Init backend implementation was added in this pass. | `server/index.js`, `server/scripts/sqlite_admin.py`, `dashboard/app.js` | The repo now includes real env verification, real SQLite status/inspect/delete/recreate-empty endpoints, and confirmation-gated destructive actions. | Use this slice as the pattern for future backend additions. |
| Cron remains intentionally blocked rather than falsely implemented. | `server/index.js`; `docs/09_CRON_AND_WATCHDOG.md` | The API now tells the truth about the scheduler/platform contradiction instead of pretending cron works. | Decide on Unix cron vs Windows Task Scheduler vs another scheduler abstraction. |
