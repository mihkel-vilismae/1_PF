...
| B Test | `dashboard/views/testView.ts`, `dashboard/app.ts`, `dashboard/services/runtimeTruth.ts`, `dashboard/services/runtimeTruth/*`, `dashboard/services/runtimeExecutionService.ts`, `docs/OLD_DOCS/VIEW_B_TEST.md`, `docs/OLD_DOCS/07_PIPELINE_STAGES.md`, `docs/OLD_DOCS/13_FRONTEND_BACKEND_CONTRACT.md` | Exercise test-only flows for login, staged pipeline runs, playback emulation, and screen simulation without touching real runtime state. | Partial | The hero says simulation only, but current code has real backend calls for B2, B3.1-B3.5, and B4 through runtime execution services. The remaining limits are important: Stage 1 download still uses mock/generated-data semantics, B3.4 geocode uses a deterministic placeholder provider, `run-b3-auto` sequences frontend calls rather than the backend orchestration endpoint, and B5 remains frontend-only screen simulation state. | Decide whether B3 auto-run should stay frontend sequential or use `/api/runtime/orchestration/run`, define a real B5 screen contract, and replace mock/generated Stage 1 download plus placeholder geocoding only if production behavior is required. | Medium | The area mixes backend-wired partial runtime stages with still-mock surfaces, and several contracts depend on backend services or provider choices that are not settled yet. |
| C Last Run info | `dashboard/views/lastRunView.ts`, `dashboard/app.ts`, `dashboard/services/runtimeTruth.ts`, `dashboard/services/runtimeTruth/*`, `docs/OLD_DOCS/VIEW_C_LAST_RUN_INFO.md`, `docs/OLD_DOCS/12_STATE_AND_RECOVERY.md`, `docs/OLD_DOCS/13_FRONTEND_BACKEND_CONTRACT.md` | Show the last durable runtime snapshot and offer a controlled restore entry point. | Partial | View-level demo buttons now explicitly force `none`, `error`, and `ready`; `seedDemoState()` fabricates last-run data; `resume-last-run` remains a labeled placeholder action. Backend orchestration `current`/`last` routes exist elsewhere, but this View C UI does not consume them and no restore flow is wired. | Decide whether `/api/runtime/orchestration/last` is the intended read source or whether a separate restore/snapshot endpoint is required, then render actual recovery evidence and replace the resume button with a real administrative restore flow. | High | Durable state, checkpoint, and recovery services are only partially represented by runtime/orchestration state today, and the dashboard restore path is not implemented. |
...
| `dashboard/services/runtimeTruth.ts` | lines 257-265, 406-533 | B1, B2, B3, B4, and B5 behavior is implemented through timers and frontend state mutations, with truth snapshots mirrored to `conf/runtime-truth.json`. | This is the core placeholder engine. |
...
| Business logic | Partial | `dashboard/services/runtimeTruth.ts` lines 303-385, 406-533 | The area has real frontend coordination logic such as duplicate guards, pipeline locks, and B5-to-B4 coupling, but action outcomes remain fake. |
...
| B5 screen simulation mutates local state only and has no explicit apply action. | `dashboard/services/runtimeTruth.ts` lines 226-245, 352-385; `dashboard/views/testView.ts` lines 105-124 | State changes are immediate in memory, so the future backend contract must decide whether B5 keeps immediate apply semantics or introduces explicit submit actions. | Send config to `POST /api/test/screen/configure`, read resulting state from `GET /api/test/screen/state`, and reconcile the UI with the intended control model. |
...
| 5 | Rework B5 to submit configuration through backend, read back resulting state, and decide whether the docs need apply buttons or the docs should be corrected. | `dashboard/views/testView.ts`, `dashboard/app.ts`, `dashboard/services/runtimeTruth.ts`, `docs/VIEW_B_TEST.md` | Screen config endpoint and UX decision | Medium |
