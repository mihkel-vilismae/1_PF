...
import {
  ...
  WHOLE_LOGIC_TEST_MODE_MANUAL_CRON_BUTTONS,
...
export async function buildWholeLogicTestModeStartResult(
  options: WholeLogicStartOptions = {},
) {
  ...
  if (runtimeMode !== "test") {
    return {
      status: "blocked",
      message:
        "Whole-logic no-login emulator start is blocked outside Test Mode.",
      runtimeMode,
  ...
  const existingState = await readExistingControllerState(
    options.controllerStateFilePath,
  );
    ...
    await appendEnd2EndLog(options.end2endLogFilePath, [
      {
        ...
        level: "warning",
        message:
          "Duplicate TEST MODE FAST EMULATOR start click was blocked because the owned controller run is already active.",
      },
    ]);
    return {
      status: "blocked",
  ...
  const writes = await writeWholeLogicRuntimeFiles({
    ...options,
    config,
    crontabText,
    controllerState,
  ...
  return {
    status: "ok",
    message:
      "Test Mode fast-emulator boundary is configured, the large start button is now disabled, and the owned controller state is running. Controls only affect this controller state; they do not kill arbitrary processes.",
    runtimeMode,
    platform: options.platform ?? process.platform,
    ...
    emulator: {
      ...
      limitation:
        "Current CronEmulator uses five-field minute cron rows; requested 6/3/12-second fast-emulator cadences are tracked by the Test Mode controller state and are not claimed as real OS cron proof.",
    },
    workerStageItemLimit: WHOLE_LOGIC_TEST_MODE_STAGE_LIMIT,
    config,
    controllerState,
...
export async function buildWholeLogicTestModeStatusResult(
  options: WholeLogicControlOptions = {},
) {
  const runtimeMode = options.runtimeMode ?? "unknown";
  if (runtimeMode !== "test") {
    return buildWholeLogicBlockedControlResult(
      runtimeMode,
      "status",
      options.now,
    );
...
export async function buildWholeLogicTestModeControlResult(
  options: WholeLogicControlOptions = {},
) {
  const runtimeMode = options.runtimeMode ?? "unknown";
  ...
  if (!key) {
    return {
      status: "blocked",
      message:
        "Whole-logic Test Mode control key must be one of q, w, e, r, or t.",
      runtimeMode,
  ...
  if (
    isManualCronControlKey(key) &&
    (!existingState?.runActive ||
      !existingState.manualCronButtons?.some(
        (button) => button.key === key && button.enabled,
      ))
  ) {
    return {
      status: "blocked",
      message:
        "Manual cronjob buttons 1, 2, and 3 are disabled until the large TEST MODE FAST EMULATOR start button has been pressed.",
      runtimeMode,
      key,
      destructiveActionAttempted: false,
      productionBehaviorChanged: false,
  ...
  const previousState =
    existingState ??
    (await readControllerState(options.controllerStateFilePath, options.now));
  ...
  const latestFocusedLog = nextState.focusedLog.at(-1);
  const end2endEntries =
    latestFocusedLog && latestFocusedLog.message !== controlLogEntry.message
      ? [controlLogEntry, latestFocusedLog]
      : [controlLogEntry];
  await appendEnd2EndLog(options.end2endLogFilePath, end2endEntries);
...
async function writeWholeLogicRuntimeFiles({
  configFilePath,
  crontabFilePath,
  ...
  crontabText,
  controllerState,
}: WholeLogicStartOptions & {
  config: unknown;
  crontabText: string;
  controllerState: WholeLogicControllerState;
}) {
  if (configFilePath) {
...
async function readControllerState(
  controllerStateFilePath: string | undefined,
  now: Date | undefined,
): Promise<WholeLogicControllerState> {
  const existing = await readExistingControllerState(controllerStateFilePath);
  return (
    existing ??
    buildWholeLogicTestModeControllerState((now ?? new Date()).toISOString())
  );
}

// Reads an existing controller state only when it is present and schema-compatible enough.
async function readExistingControllerState(
  controllerStateFilePath: string | undefined,
): Promise<WholeLogicControllerState | null> {
  if (!controllerStateFilePath) {
    return null;
  }
  try {
    const parsed = JSON.parse(
      await fs.readFile(controllerStateFilePath, "utf8"),
    );
    if (
      parsed &&
      typeof parsed === "object" &&
      parsed.mode === "test-mode-whole-logic-controller"
    ) {
...
async function writeEnd2EndLog(
  targetPath: string | undefined,
  entries: WholeLogicFocusedLogEntry[],
): Promise<void> {
  if (!targetPath) {
    return;
  }
...
async function appendEnd2EndLog(
  targetPath: string | undefined,
  entries: WholeLogicFocusedLogEntry[],
): Promise<void> {
  if (!targetPath) {
...
function formatEnd2EndLog(entries: WholeLogicFocusedLogEntry[]): string {
  return (
    entries
      .map(
        (entry) =>
          `${entry.at} [${entry.level.toUpperCase()}] ${sanitizeEnd2EndLogMessage(entry.message)}`,
      )
...
function applyWholeLogicControl(
  state: WholeLogicControllerState,
  key: WholeLogicControlKey,
  nowIso: string,
  ...
  next.events.push({
    at: nowIso,
    key,
    action: WHOLE_LOGIC_TEST_MODE_CONTROL_ACTIONS[key].label,
  });
  next.focusedLog.push({
    at: nowIso,
    level: "info",
    message: `Control ${key.toUpperCase()} applied to the owned Test Mode controller.`,
  });

  if (isManualCronControlKey(key)) {
    markManualCronjobCalled(next, key, nowIso);
  } else if (key === "q") {
    terminateWorker(
      next,
      "regularStage",
      nowIso,
      "SIGKILL_SIMULATED_POWER_LOSS",
    );
  } else if (key === "w") {
    ...
    next.startButton = {
      ...
      disabled: true,
      reason:
        "Power-off simulation is active; press T again to re-enable cronjobs.",
    };
    terminateWorker(
      next,
      "regularStage",
    ...
    restarted.events = [
      ...next.events,
      {
        ...
        key,
        action:
          "Power-on simulation re-enabled cronjobs and worker controller records.",
      },
    ];
    restarted.focusedLog = [
      ...next.focusedLog,
      {
        at: nowIso,
        level: "success",
        message:
          "Power-on simulation re-enabled cronjobs and owned worker records.",
      },
    ];
    return restarted;
  }
...
// Checks whether a controller key is one of the manual cronjob call buttons.
function isManualCronControlKey(
  key: WholeLogicControlKey,
): key is keyof typeof WHOLE_LOGIC_TEST_MODE_MANUAL_CRON_BUTTONS {
  return key === "1" || key === "2" || key === "3";
}

// Records one manual cronjob call in controller-owned state and focused logs.
function markManualCronjobCalled(
  state: WholeLogicControllerState,
  key: keyof typeof WHOLE_LOGIC_TEST_MODE_MANUAL_CRON_BUTTONS,
  nowIso: string,
): void {
  const button = WHOLE_LOGIC_TEST_MODE_MANUAL_CRON_BUTTONS[key];
  state.statusRows = state.statusRows.map((row) => {
    if (row.id !== button.statusId) {
      return row;
    }
    return {
      ...row,
      state: "passed",
      firstCalledAt: row.firstCalledAt ?? nowIso,
      lastCalledAt: nowIso,
      calledCount: row.calledCount + 1,
      message: button.resultMessage,
    };
  });
  state.focusedLog.push({
    at: nowIso,
    level: "success",
    message: `Manual button ${key} called ${button.label === "1" ? "regular" : button.label === "2" ? "playback" : "screen on-off"} worker cronjob boundary.`,
  });
}
...
function terminateWorker(
  state: WholeLogicControllerState,
  workerKey: keyof WholeLogicControllerState["workers"],
  nowIso: string,
  signal: string,
): void {
...
function buildWholeLogicBlockedControlResult(
  runtimeMode: RuntimeMode,
  key: unknown,
  now: Date | undefined,
) {
...
export const WHOLE_LOGIC_TEST_MODE_DEFAULT_END2END_LOG_PATH =
  WHOLE_LOGIC_TEST_MODE_END2END_LOG_RELATIVE_PATH;
