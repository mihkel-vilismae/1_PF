...
function buildEvents(input: { qResult: QCreatedQueueResult; executedAt: string }): Array<Record<string, unknown>> {
  ...
  return [
    ...
    event('GPS parser', q.metadataAddress.gpsProcessed > 0 ? 'finished' : 'interrupted', input.executedAt, 'DEMO GPS metadata stage completed/degraded', { processed: q.metadataAddress.gpsProcessed, success: q.metadataAddress.gpsSuccess, failed: q.metadataAddress.gpsFailure }),
    event('Geocode', q.metadataAddress.geocodeProcessed > 0 ? 'finished' : 'interrupted', input.executedAt, 'DEMO geocode provider stage completed/degraded', { processed: q.metadataAddress.geocodeProcessed, success: q.metadataAddress.geocodeSuccess, failed: q.metadataAddress.geocodeFailure }),
