...
const assertions = {
  ...
  direct_route_plans_not_runs_by_default: direct.status === 'planned' && direct.messages.some((message) => message.includes('PHOTOFRAME_TERMINAL_DEMO_EXECUTE=1')),
  modal_key_2_marks_planned: modal.rows.find((candidate) => candidate.key === '2')?.status === 'planned',
...
console.log(JSON.stringify({
  ...
  decision: passed ? 'START_STAGE_MODAL_SHARED_WORKER_PATH_READY' : 'START_STAGE_MODAL_SHARED_WORKER_PATH_BLOCKED',
  assertions,
  latestEvent: latest
}, null, 2));
