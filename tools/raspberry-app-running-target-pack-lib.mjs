...
export async function buildRaspberryAppRunningTargetPackProof({ metadata, env = process.env, repoRoot = process.cwd(), commandRunner = runCommand } = {}) {
  ...
  return createProofEnvelope({
    ...
    evidence: sanitizeEvidence({
      ...
      evaluation,
      pass_criteria: 'PASSED only on a non-override Raspberry target when target tools/fixtures, executable/env repair, worker startup smoke, cron install, app-running pass harness, worker evidence/app-running chain, and native image/video playback all report PASSED. The app-running pass harness intentionally runs before worker-evidence-dependent checks so complete duplicate/stale-lock evidence exists first.',
