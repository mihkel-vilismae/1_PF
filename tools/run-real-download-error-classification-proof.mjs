...
import { evaluateRealDownloadErrorClassification } from './real-download-error-classification-lib.mjs';
async function meta() {
...
const evaluation = evaluateRealDownloadErrorClassification(process.env, { cwd: process.cwd() });
const { version, gitCommit } = await meta();
const envelope = createProofEnvelope({
  proofKind: 'real_download_error_classification',
  ...
  knownLimitations: ["Classifies blocked/failure categories; no provider call."],
});
const outputPath = await writeProofArtifact('real_download_error_classification', envelope);
