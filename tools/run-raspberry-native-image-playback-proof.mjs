/**
 * Raspberry native image playback proof runner for PF_login.
...
import { buildRaspberryNativeImagePlaybackProof } from './raspberry-native-image-playback-proof-lib.mjs';
...
async function main() {
  const envelope = await buildRaspberryNativeImagePlaybackProof({ metadata: await readProjectMetadata() });
  const outputPath = await writeProofArtifact('raspberry_native_image_playback', envelope);
