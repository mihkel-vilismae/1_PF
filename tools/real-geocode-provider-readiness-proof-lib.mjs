...
import { buildProviderSafetyReadiness } from './real-geocode-provider-adapter-lib.mjs';
...
function readinessChecks(env = process.env) {
  ...
  const checks = [
    ...
    { name: 'provider_id_configured', passed: Boolean(providerId), detail: 'PF_GEOCODE_CHAIN_PROOF_PROVIDER or PF_GEOCODE_PROOF_PROVIDER must be set' },
    ...
    { name: 'provider_safety_configured', passed: providerSafety.proof_status === 'PASSED', detail: providerSafety.checks.map((check) => ({ name: check.name, passed: check.passed })) },
...
export function buildRealGeocodeProviderReadinessProof({ metadata, env = process.env } = {}) {
  ...
  return createProofEnvelope({
    ...
    knownLimitations: proofStatus === 'PASSED'
      ? ['This only proves local configuration readiness and provider-safety configuration; it does not call or prove a real geocode provider.']
      : ['Provide explicit real geocode opt-in, a supported provider id, and required provider-specific safety/configuration before running the provider-chain proof.'],
  });
}
