/**
 * Raspberry generated fixture proof runner.
 * Writes sanitized JSON evidence under runtime_data/proofs.
 */
import { readFile } from 'node:fs/promises';
import { runCommand, writeProofArtifact } from './proof-utils.mjs';
import { buildRaspberryGeneratedFixtureProof } from './raspberry-generated-fixture-proof-lib.mjs';
...
async function main() {
  const envelope = await buildRaspberryGeneratedFixtureProof({ metadata: await readProjectMetadata() });
  const outputPath = await writeProofArtifact('raspberry_generated_fixture_validation', envelope);
