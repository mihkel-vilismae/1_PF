/** Raspberry cron worker runtime proof runner. */
...
async function main() {
  const envelope = await buildRaspberryCronWorkerRuntimeProof({ metadata: await readProjectMetadata() });
  const outputPath = await writeProofArtifact('raspberry_cron_worker_runtime', envelope);
