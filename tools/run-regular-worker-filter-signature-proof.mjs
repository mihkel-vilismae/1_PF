...
import { evaluateRegularWorkerFilterSignature } from './regular-worker-filter-signature-lib.mjs';
async function meta() {
...
const evaluation = evaluateRegularWorkerFilterSignature(process.env, { cwd: process.cwd() });
const { version, gitCommit } = await meta();
const envelope = createProofEnvelope({
  proofKind: 'regular_worker_filter_signature',
  ...
  knownLimitations: ["Checks worker/filter signature alignment with manifest."],
});
const outputPath = await writeProofArtifact('regular_worker_filter_signature', envelope);
