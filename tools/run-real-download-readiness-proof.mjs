/** Real download continuation readiness preflight proof runner. */
import { readFile } from 'node:fs/promises';
import { runCommand, writeProofArtifact } from './proof-utils.mjs';
import { buildRealDownloadReadinessProof } from './real-download-readiness-proof-lib.mjs';
...
async function main() {
  ...
  const envelope = buildRealDownloadReadinessProof({
    metadata: await readProjectMetadata(),
    baseUrl,
    recentCount: Number.isFinite(recentCount) ? recentCount : 10,
  });
  const outputPath = await writeProofArtifact('real_download_readiness', envelope);
