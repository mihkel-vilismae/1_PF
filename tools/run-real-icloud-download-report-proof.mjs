...
const evaluation = evaluateRealIcloudDownloadReport(process.env, { cwd: process.cwd() });
const { version, gitCommit } = await meta();
const envelope = createProofEnvelope({
  proofKind: 'real_icloud_download_report',
  ...
  knownLimitations: ["Summary proof; remains BLOCKED until all real evidence gates pass."],
});
const outputPath = await writeProofArtifact('real_icloud_download_report', envelope);
