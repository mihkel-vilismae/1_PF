# Windows live native playback proof runner for PF_login / 1234_PF.
# Starts a proof-only API process with native playback env values in a temp env file.
# Runs proof:live-windows-native-playback against the actual API port used by this runner.
...
$GeneratedEnvPath = Join-Path $RepoRoot "runtime_data\live_windows_native_playback_proof.env"
$ApiStdoutPath = Join-Path $RepoRoot "logs\live_windows_native_playback_api.stdout.log"
$ApiStderrPath = Join-Path $RepoRoot "logs\live_windows_native_playback_api.stderr.log"
...
function New-ProofEnvironmentFile {
    ...
    $overrides = @(
        "",
        "# Proof-only overrides generated by start_live_windows_native_playback_proof.cmd.",
...
<#
Runs the live native playback proof against the API started by this script.
#>
function Invoke-LiveNativeProof {
    ...
    try {
        ...
        Invoke-RepoCommand -FilePath "npm" -Arguments @("run", "proof:live-windows-native-playback") -StepName "Run live Windows native playback proof"
    }
    finally {
...
function Export-EvidenceZip {
    ...
    $outDir = Join-Path $downloads "PF_login_native_playback_evidence_$stamp"
    $zipPath = "$outDir.zip"
    ...
    $latestProof = Get-ChildItem (Join-Path $RepoRoot "runtime_data\proofs\live_windows_native_playback_*.json") -ErrorAction SilentlyContinue |
        Sort-Object LastWriteTime -Descending |
        Select-Object -First 1

    $summaryPath = Join-Path $outDir "summary.txt"
    @"
PF_login live Windows native playback proof bundle
...
$apiProcess = $null
try {
    Write-ProofStep "PF_login live Windows native playback proof launcher."
    Push-Location $RepoRoot
    try {
        ...
        if ($script:ProofBlockedByMpvInstall) {
            Invoke-RepoCommand -FilePath "npm" -Arguments @("run", "proof:live-windows-native-playback") -StepName "Write honest BLOCKED proof for blocked mpv install"
            Export-EvidenceZip
            return
        }
        ...
        try {
            ...
            $proofFailure = $_
            Write-ProofStep "Live native playback proof failed; exporting evidence before exiting." "Yellow"
        }
        Export-EvidenceZip
