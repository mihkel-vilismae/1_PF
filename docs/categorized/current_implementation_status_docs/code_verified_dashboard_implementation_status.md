...
Update note: 2026-05-12 18:34 Tallinn — docs reconciled after NEW AUTH passive skipped-proof UX, active provider-verification action, and redaction regression coverage. Earlier 2026-05-10 B4 playback/CronEmulator notes remain valid.
...
| View A New Auth | `new-auth-*` controls and `/api/auth/new/*` | Implemented with provider-dependent runtime proof | UI renders new-auth controls; frontend service targets only `/api/auth/new/*`; backend route family covers status, verify iCloudPD install, session-files, login, submit-2FA, logout, and test-download proof. Passive skipped provider proof is now an actionable UI state, and `Verify with iCloudPD` runs active provider proof through `GET /api/auth/new/status`. | Runtime success still depends on local iCloudPD, Apple account state, and real provider response. Local session files alone are not authenticated. Passive status remains read-only; install readiness verification is not session proof. | `dashboard/views/initView.ts`; `dashboard/services/newAuthService.ts`; `dashboard/services/runtimeTruth/runtimeTruthNewAuthActions.ts`; `server/index.ts`; `server/auth/newAuthRoutes.ts`; `server/auth/newAuthService.ts`; `tests/newAuth*.test.js`; `tests/newAuthSlice*.mjs` | Targeted NEW AUTH tests; build; docs reconciliation |
...
| View C status was undocumented in button-verification corpus. | `docs/categorized/current_implementation_status_docs/button_and_view_verification_status.md:62`. | Code shows `refresh-last-run` consumes `/api/runtime/orchestration/last` to load a read-only last-run summary while `resume-last-run` remains a placeholder with no backend call. | `Partial (read-only backend last-run read; restore remains mock-only)`. |
...
| View C says no real `/api/runtime/*` endpoint is wired. | Previous copy in `dashboard/views/lastRunView.ts` implied no runtime endpoints. | The updated View C copy explicitly references `GET /api/runtime/orchestration/last` and reads a read-only summary from SQLite. There is still no restore endpoint, and the resume button remains disabled as a placeholder. | Update docs to reflect that View C partially consumes backend runtime endpoints for read-only last-run data while restore remains mock-only. |
...
## 2026-05-12 NEW AUTH provider-verification UX update

The current implementation status includes the Slice 1-3 login reconciliation work. The relevant code-verified behavior recorded by the implementation slices is:

| Behavior | Status | Evidence |
|---|---|---|
| Passive skipped-proof mapping | Implemented | `NEW_AUTH_PROVIDER_PROOF_SKIPPED` maps to `Session files found, provider verification not run yet.` |
| Active provider proof action | Implemented | `new-auth-verify-provider-session` / `Verify with iCloudPD` calls active `GET /api/auth/new/status`. |
| Passive endpoint separation | Implemented | Focused tests cover that `Verify with iCloudPD` does not call `GET /api/auth/new/status?mode=passive`. |
| Install/readiness separation | Implemented | `Verify iCloudPD install` remains `POST /api/auth/new/verify-icloudpd` and is not treated as authenticated login. |
| Shared request/event logging | Implemented | Active provider-verification action uses the shared frontend API/request path. |
| Provider-output redaction | Implemented/hardened | Slice 3 added coverage and sanitizer hardening for provider communication fields. |

Focused verification reported by Slice 3:
...
The full `npm run test` command was attempted during Slice 3 but timed out in the execution environment after initial auth tests passed. Treat this as an environment/runtime limitation until a full local run is completed, not as a change in documented product behavior.
