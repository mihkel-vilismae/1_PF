...
|---|---|---|---|---|---|---|---|---|---|
| A | 1A-TEST-WHOLE-LOGIC | RUN whole logic without logging in | Test Mode-only operator surface for the planned whole-logic scheduler/emulator flow without requiring login. | High | INSTALL CRONTAB/EMULATOR, CALLING REGULAR WORKER EVERY 1 MINUTES, PLAYBACK WORKER EVERY 30sec, screen on-off worker EVERY 2 MINUTES, ADD LIMIT OF 5 ITEMS TO EACH WORKER STAGE (INCLUDING THE MOCK DOWNLOAD) | `data-action="run-whole-logic-test-mode"` | Group 2 backend boundary wired. Focused tests prove it renders only in Test Mode and the button is enabled for the safe Test Mode endpoint. | Documented in `docs/20_architecture_and_specs/test_mode_whole_logic_emulator_contract.md`. | Correct for Group 2; starts only the safe backend configuration boundary, while process controls remain Group 3. |
