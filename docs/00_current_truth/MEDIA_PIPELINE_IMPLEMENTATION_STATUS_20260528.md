...
### Draft prompt
Using the Goal 4 inventory, behavior analysis, and verification alignment, create the final implementation-status table for Download, Index, GPS parser, Geocode, Queue / Q, and Playback Select.
...
| GPS parser | Implemented. Docs/UI describe B3.3 as `POST /api/runtime/gps/run`. | Implemented / covered. Stage 3 reads queued canonical files, extracts EXIF GPS, marks GPS-found/no-GPS rows, and seeds geocode queue for successful assets. Wave D verifies GPS-found and GPS-not-found outcomes. | Pending user PC testing. | `server/database/databaseService.ts` `runStage3ProcessGpsQueue`; `server/scripts/sqlite_admin.py stage3_process_gps_queue`; `tests/waveD.e2e.test.js`; `tests/waveE.step5.test.js`. | Verified against controlled EXIF fixtures. Parser breadth and unusual camera metadata variants may need later test data. |
| Geocode | Implemented / bounded. Docs/UI correctly warn that B3.4 uses deterministic placeholder geocoder only. | Implemented / covered for deterministic placeholder geocode. Stage 4 writes `Lat: ..., Lon: ...` address text and address cache rows. Wave D verifies geocode success for GPS-found assets and pending/not-ready state for no-GPS asset. | Pending user PC testing. | `server/database/databaseService.ts` `runStage4ProcessGeocodeQueue`; `server/scripts/sqlite_admin.py stage4_process_geocode_queue`; `tests/waveD.e2e.test.js`; `tests/waveE.step5.test.js`; `dashboard/views/testView.ts`. | Not production reverse geocoding. A real provider, rate limiting, retries, network failure handling, and address quality are future work. |
| Queue / Q | Implemented. Docs/UI describe B3.5 as `POST /api/runtime/queue/prepare`. | Implemented / covered. Stage 5 inserts only eligible geocoded assets with usable variants and skips ineligible assets with structured reasons. Wave D verifies `already_queued`, `geocode_not_ready`, `missing_variant`, and `missing_file_path` behavior and idempotent second run. | Pending user PC testing. | `server/database/databaseService.ts` `runStage5PrepareQueue`; `server/scripts/sqlite_admin.py prepare_slideshow_queue`; `tests/waveD.e2e.test.js`; `tests/viewB.buttonWorkflow.test.js`. | Eligibility rules are intentionally strict. Future queue policies such as prioritization, retention, or unresolved-asset policy must update tests/docs together. |
...
| GPS parser | Did GPS-found/no-GPS outcomes match the actual media files? |
| Geocode | Did address text appear, and was it clear that the current address is placeholder `Lat/Lon` text? |
| Queue / Q | Did eligible items enter the queue, and did skipped items show understandable reasons? |
...
The deterministic backend media pipeline is implemented and covered by repo tests from Download through Playback Select. The honest limitations are Real Mode iCloudPD production download dependency, deterministic placeholder geocoding, absent View C restore mutation, and the need for Mihkel's PC-tested subjective assessment before the subjective column is filled.
