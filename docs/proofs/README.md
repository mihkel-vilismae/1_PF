...
| Real iCloud media source evidence pack | `npm run proof:real-icloud-media-source-evidence-pack` | Generates redacted auth/download/manifest/continuation templates, latest.env, and BLOCKED-safe operator next steps for real iCloud media source evidence. See [`real_icloud_media_source_evidence_pack_proof.md`](real_icloud_media_source_evidence_pack_proof.md). |
...
| Regular worker product evidence producer | `npm run proof:regular-worker-product-evidence-producer` | Validates proof-safe regular-worker product evidence production without replacing target-machine proof. |
...
| Fedora cron preflight | `npm run proof:linux-fedora-cron-preflight` | Checks rehearsal scheduler prerequisites without proving active cron execution. |
| Fedora worker singleton pack | `npm run proof:linux-fedora-worker-singleton-pack` | Checks rehearsal worker singleton/lock behavior without Raspberry target claims. |
| Fedora product pipeline rehearsal | `npm run proof:linux-fedora-product-pipeline-rehearsal` | Runs the retained rehearsal path without proving real production media behavior. |
...
## Current package proof script registry

This generated registry keeps package proof script references discoverable and synchronized for documentation tests. Detailed proof docs above remain the source for interpretation and target-machine boundaries.
...
| `npm run proof:auth-checkpoint-operator-flow` |
| `npm run proof:auth-checkpoint-state` |
| `npm run proof:auth-checkpoint-state:test` |
| `npm run proof:auth-operator-2fa-checkpoint` |
| `npm run proof:auth-session-snapshot-contract` |
| `npm run proof:auth-session-usable-evidence` |
| `npm run proof:auth-session-usable-evidence-producer` |
| `npm run proof:controlled-restore-contract` |
| `npm run proof:debug-page-keybook` |
| `npm run proof:debug-page-keybook-render` |
| `npm run proof:debug-page-runtime` |
| `npm run proof:debug-page-style-contract` |
| `npm run proof:debug-page-world-class-completion` |
| `npm run proof:debug-page-world-class-plan` |
| `npm run proof:debug-page-world-class-reconciliation` |
| `npm run proof:deterministic-media-pipeline` |
...
| `npm run proof:download-partial-file-safety` |
| `npm run proof:e2e-local-photo-frame` |
| `npm run proof:full-test` |
| `npm run proof:geocode-dry-run-from-sample` |
| `npm run proof:geocode-provider` |
| `npm run proof:geocode-provider-selection` |
| `npm run proof:gps-fallback` |
| `npm run proof:icloud-filter-signature` |
| `npm run proof:icloudpd-preflight-boundary` |
| `npm run proof:icloudpd-session-path-validator` |
| `npm run proof:linux-fedora-cron-preflight` |
| `npm run proof:linux-fedora-env-preflight` |
| `npm run proof:linux-fedora-executable-permissions` |
...
| `npm run proof:live-windows-native-playback:windows` |
| `npm run proof:live-windows-native-recovery` |
| `npm run proof:live-windows-native-recovery:windows` |
| `npm run proof:live-windows-native-video-playback` |
| `npm run proof:live-windows-native-video-playback:windows` |
| `npm run proof:live-windows-scheduler` |
| `npm run proof:live-windows-scheduler:windows` |
| `npm run proof:local-generated-media-pipeline-rehearsal` |
| `npm run proof:native-fullscreen-playback` |
| `npm run proof:openspec-v1-audit` |
| `npm run proof:operator-safe-icloud-session-checkpoint` |
| `npm run proof:operator-status-real-download` |
| `npm run proof:overall-project-completeness-registry` |
| `npm run proof:proof-report-blocker-summary` |
| `npm run proof:proof-run-handoff-triage` |
| `npm run proof:proof-runner-final-summary` |
| `npm run proof:proof-runner-queue` |
| `npm run proof:prooflauncher-gui-selection` |
| `npm run proof:prooflauncher-logs-zip-hygiene` |
| `npm run proof:prooflauncher-logs-zip-hygiene-contract` |
| `npm run proof:prooflauncher-timing-history` |
| `npm run proof:proofrunner-handoff-artifact-export-contract` |
| `npm run proof:proofrunner-handoff-runtime-contract` |
| `npm run proof:proofrunner-launcher-progress-contract` |
| `npm run proof:proofrunner-packaging-identity` |
| `npm run proof:proofrunner-platform-filter-contract` |
| `npm run proof:proofrunner-powershell-safe-log-name` |
| `npm run proof:proofrunner-tiered-queues` |
| `npm run proof:proofrunner-windows-launcher-contract` |
| `npm run proof:provider-env-template` |
...
| `npm run proof:raspberry-dashboard-status-view` |
...
| `npm run proof:real-download-error-classification` |
| `npm run proof:real-download-readiness` |
| `npm run proof:real-geocode-provider-chain` |
| `npm run proof:real-geocode-provider-readiness` |
| `npm run proof:real-gps-geocode-product-bridge` |
| `npm run proof:real-gps-geocode-product-bridge-evidence-pack` |
| `npm run proof:real-icloud-batch1-producer` |
| `npm run proof:real-icloud-batch2-producer` |
| `npm run proof:real-icloud-controlled-download-dir` |
| `npm run proof:real-icloud-download-no-loop` |
| `npm run proof:real-icloud-download-preflight` |
| `npm run proof:real-icloud-download-report` |
| `npm run proof:real-icloud-evidence-run-package` |
| `npm run proof:real-icloud-evidence-zip-contract` |
| `npm run proof:real-icloud-filter-config` |
...
| `npm run proof:real-icloud-listing-preflight` |
| `npm run proof:real-icloud-media-source-evidence-pack` |
| `npm run proof:real-icloud-no-loop-producer` |
| `npm run proof:real-icloud-proof-runbook` |
| `npm run proof:real-icloudpd` |
| `npm run proof:real-icloudpd-readiness` |
| `npm run proof:real-provider-artifact-redaction` |
| `npm run proof:regular-worker-consumes-download-manifest` |
| `npm run proof:regular-worker-filter-signature` |
| `npm run proof:regular-worker-product-contract` |
| `npm run proof:regular-worker-product-evidence-producer` |
| `npm run proof:regular-worker-product-evidence-template` |
| `npm run proof:regular-worker-real-download-bridge` |
| `npm run proof:runtime-state-durable-checkpoint` |
| `npm run proof:runtime-status-projection` |
| `npm run proof:scheduler-host-boundary` |
| `npm run proof:scheduler-host-mock` |
| `npm run proof:screen-worker-nonblocking-design` |
| `npm run proof:test-mode-whole-logic-emulator` |
| `npm run proof:v1-readiness-data-requirements` |
...
| `npm run proof:v2-install-production-crontab` |
| `npm run proof:v2-install-production-crontab-contract` |
| `npm run proof:v2-install-real-crontab` |
| `npm run proof:v2-production-cron-cleanup` |
| `npm run proof:v2-production-cron-runtime` |
| `npm run proof:v2-production-cron-runtime-contract` |
| `npm run proof:v2-real-cron-cleanup` |
...
| `npm run proof:v2-target-live-readiness` |
| `npm run proof:v2-visual-physical-evidence` |
| `npm run proof:v2-visual-physical-evidence-contract` |
| `npm run proof:verify-generated-test-data` |
| `npm run proof:view-a-mode-safety` |
| `npm run proof:view-a-refresh-plan` |
| `npm run proof:view-c-readonly-wording` |
| `npm run proof:windows-cronemulator` |
| `npm run proof:windows-live-proof-blocked-contract` |
