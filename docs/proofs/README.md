...
The table above is intentionally complete for current `proof:*` package scripts. As of this repository version, `package.json` exposes 61 proof package scripts and every one must appear in this table. `tests/docsNpmScriptReferences.test.js` guards active documentation against stale `npm run ...` script references and against missing proof-runner inventory entries.
...
`npm run proof:raspberry-cron-worker-runtime` implements the target-gated Raspberry app-running proof runner. It requires active managed cron rows and worker evidence proving invocation, same-worker singleton duplicate-skip, cross-worker independence, and stale-lock reclaim for `regular_stage_worker`, `playback_worker`, and `screen_on_off_worker`. It can auto-load `runtime_data/raspberry_worker_evidence/latest.json` generated by `npm run proof:raspberry-worker-evidence`; `PF_RASPBERRY_CRON_WORKER_EVIDENCE_FILE` remains an explicit override. See [`raspberry_cron_worker_runtime_proof.md`](raspberry_cron_worker_runtime_proof.md).
