...
| `docs/10_runbooks/master_branch_roundtrip_workflow.md` | current_latest_baseline | Reusable runbook for splitting from `master`, maintaining branch handoff metadata, and uploading sibling branch ZIPs together before merge-back; workflow guidance only, not merge proof. |
...
| `docs/20_architecture_and_specs/openspec/terminal_demo_v2_operator_rc_handoff_openspec.md` | `current_truth` | v1.9.1 docs-only handoff for implementing Terminal Demo v2.0.0 operator RC; documentation/proof-contract only until v2.0 proof pack passes. |
