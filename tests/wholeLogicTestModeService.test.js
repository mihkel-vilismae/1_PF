...
test("whole-logic start records requested cadences, max-5 limit, disabled button, and owned records", async () => {
  ...
  assert.equal(
    result.config.workers.regularStage.includesMockDownloadLimit,
    true,
  );
  ...
  assert.equal(result.controllerState.manualCronButtons.length, 3);
  assert.equal(
    result.controllerState.manualCronButtons.every((button) => button.enabled),
    true,
  );
  assert.equal(
    result.controllerState.statusRows.find(
      (row) => row.id === "crontab_working",
    ).state,
    "passed",
  );
  assert.equal(
    result.controllerState.statusRows.find(
      (row) => row.id === "native_playback_started",
    ).state,
    "pending",
  );
  assert.match(
    result.controllerState.focusedLog.at(-1).message,
    /Native playback/,
  );
...
test("whole-logic start writes runtime config, CronEmulator crontab, controller state, and end2end_test.log", async () => {
  ...
  try {
    const configPath = path.join(
      dir,
      "scheduler",
      "whole-logic-test-mode-config.json",
    );
    const crontabPath = path.join(dir, "cron", "crontab_emulated.txt");
    const controllerPath = path.join(
      dir,
      "scheduler",
      "whole-logic-test-mode-controller.json",
    );
    ...
    assert.equal(result.status, "ok");
    assert.equal(
      await readFile(crontabPath, "utf8"),
      buildWholeLogicWindowsCronEmulatorCrontabText(),
    );
...
test("duplicate start is blocked after the first click and preserves disabled state", async () => {
  ...
  try {
    ...
    const logPath = path.join(dir, "logs", "end2end_test.log");
    const first = await buildWholeLogicTestModeStartResult({
      runtimeMode: "test",
      controllerStateFilePath: controllerPath,
      end2endLogFilePath: logPath,
    });
    const second = await buildWholeLogicTestModeStartResult({
      runtimeMode: "test",
      controllerStateFilePath: controllerPath,
      end2endLogFilePath: logPath,
    });
    ...
    assert.match(
      logText,
      /Duplicate TEST MODE FAST EMULATOR start click was blocked/,
    );
  } finally {
    await rm(dir, { recursive: true, force: true });
  }
...
test("manual cronjob buttons are blocked before start and record calls after start", async () => {
  const dir = await mkdtemp(path.join(tmpdir(), "pf-whole-logic-manual-"));
  try {
    const controllerPath = path.join(dir, "controller.json");
    const logPath = path.join(dir, "logs", "end2end_test.log");
    const blocked = await buildWholeLogicTestModeControlResult({
      runtimeMode: "test",
      controllerStateFilePath: controllerPath,
      end2endLogFilePath: logPath,
      key: "1",
    ...
    assert.match(
      blocked.message,
      /disabled until the large TEST MODE FAST EMULATOR start button/,
    );

    await buildWholeLogicTestModeStartResult({
      runtimeMode: "test",
      controllerStateFilePath: controllerPath,
      end2endLogFilePath: logPath,
    });
    const firstCall = await buildWholeLogicTestModeControlResult({
      runtimeMode: "test",
      controllerStateFilePath: controllerPath,
      end2endLogFilePath: logPath,
      key: "1",
    ...
    const secondCall = await buildWholeLogicTestModeControlResult({
      runtimeMode: "test",
      controllerStateFilePath: controllerPath,
      end2endLogFilePath: logPath,
      key: "2",
      now: new Date("2026-06-02T03:00:01.000Z"),
    });
    const thirdCall = await buildWholeLogicTestModeControlResult({
      runtimeMode: "test",
      controllerStateFilePath: controllerPath,
      end2endLogFilePath: logPath,
      key: "3",
      now: new Date("2026-06-02T03:00:02.000Z"),
    });
    const regularRow = firstCall.controllerState.statusRows.find(
      (row) => row.id === "regular_worker_called",
    );
    const playbackRow = secondCall.controllerState.statusRows.find(
      (row) => row.id === "playback_worker_called",
    );
    const screenRow = thirdCall.controllerState.statusRows.find(
      (row) => row.id === "screen_on_off_worker_called",
    );
    const logText = await readFile(logPath, "utf8");

    assert.equal(firstCall.status, "ok");
    assert.equal(
      firstCall.message,
      "Manual regular worker cronjob call was recorded by the owned Test Mode controller.",
    );
    assert.equal(regularRow.calledCount, 2);
    assert.equal(regularRow.state, "passed");
    assert.equal(regularRow.lastCalledAt, "2026-06-02T03:00:00.000Z");
    assert.equal(playbackRow.calledCount, 2);
    assert.equal(screenRow.calledCount, 2);
    assert.match(
      logText,
      /Manual button 1 called regular worker cronjob boundary/,
    );
    assert.match(
      logText,
      /Manual button 2 called playback worker cronjob boundary/,
    );
    assert.match(
      logText,
      /Manual button 3 called screen on-off worker cronjob boundary/,
    );
  } finally {
    await rm(dir, { recursive: true, force: true });
  }
...
test("q/w/e controls terminate only the matching owned worker state", async () => {
  ...
  try {
    ...
    const qResult = await buildWholeLogicTestModeControlResult({
      runtimeMode: "test",
      controllerStateFilePath: controllerPath,
      key: "q",
    });
    assert.equal(
      qResult.controllerState.workers.regularStage.processState,
      "terminated",
    );
    assert.equal(
      qResult.controllerState.workers.playback.processState,
      "running",
    );
    assert.equal(
      qResult.controllerState.workers.screenOnOff.processState,
      "running",
    );
    assert.equal(qResult.destructiveActionAttempted, false);

    const wResult = await buildWholeLogicTestModeControlResult({
      runtimeMode: "test",
      controllerStateFilePath: controllerPath,
      key: "w",
    });
    assert.equal(
      wResult.controllerState.workers.playback.processState,
      "terminated",
    );
    assert.equal(
      wResult.controllerState.workers.screenOnOff.processState,
      "running",
    );

    const eResult = await buildWholeLogicTestModeControlResult({
      runtimeMode: "test",
      controllerStateFilePath: controllerPath,
      key: "e",
    });
    assert.equal(
      eResult.controllerState.workers.screenOnOff.processState,
      "terminated",
    );
    assert.equal(
      eResult.controllerState.workers.screenOnOff.lastSignal,
...
test("r stops cronjobs and t toggles power-off then power-on without killing dashboard/system processes", async () => {
  ...
  try {
    ...
    const rResult = await buildWholeLogicTestModeControlResult({
      runtimeMode: "test",
      controllerStateFilePath: controllerPath,
      key: "r",
    });
    ...
    const offResult = await buildWholeLogicTestModeControlResult({
      runtimeMode: "test",
      controllerStateFilePath: controllerPath,
      key: "t",
    });
    ...
    assert.equal(
      offResult.controllerState.databaseState,
      "abruptly_interrupted",
    );
    assert.equal(
      offResult.controllerState.playbackState,
      "abruptly_interrupted",
    );
    assert.ok(
      offResult.controllerState.safeTerminationBoundary.includes(
    ...
    const onResult = await buildWholeLogicTestModeControlResult({
      runtimeMode: "test",
      controllerStateFilePath: controllerPath,
      key: "t",
    });
    ...
    assert.equal(
      onResult.controllerState.workers.regularStage.processState,
      "running",
    );
  } finally {
    await rm(dir, { recursive: true, force: true });
...
test("whole-logic controls and status are blocked outside Test Mode", async () => {
  const controlResult = await buildWholeLogicTestModeControlResult({
    runtimeMode: "real",
    key: "q",
  });
  const statusResult = await buildWholeLogicTestModeStatusResult({
    runtimeMode: "real",
  });
