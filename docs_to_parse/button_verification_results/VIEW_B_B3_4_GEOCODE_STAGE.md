...
Root cause:
- implementation explicitly uses deterministic placeholder geocoder (`server/index.js` geocode-stage payload/notes), which is not full real-equivalent behavior.
...
| 7. Mock / Reality Validation | Pass | `guideCopy.json` now marks B3.4 real. | Matches backend wiring. |
| 8. Inspect System Alignment | Pass | Backend-status copy now marks B3.4 backend support as real. | Metadata drift resolved for this control. |
